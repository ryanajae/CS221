'''
The Driverless Car project was developed at Stanford, primarily by
...
class ExactInference(object):
    ...
    def elapseTime(self):
        ...
        elapseGrid = [[0 for _ in range(self.belief.getNumCols())] for _ in range(self.belief.getNumRows())]
        for ((oldTile, newTile), transProb) in self.transProb.iteritems():
        	elapseGrid[newTile[0]][newTile[1]] += self.belief.getProb(oldTile[0], oldTile[1]) * transProb
        self.belief.grid = elapseGrid
        self.belief.normalize()
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for ((row, col), value) in self.particles.iteritems():
        	mean = math.sqrt(((util.rowToY(row) - agentY)**2) + ((util.colToX(col) - agentX)**2))
        	newP = util.pdf(mean, Const.SONAR_STD, observedDist)
        	weights.update({(row, col) : newP * value})
        resample = collections.defaultdict(int)
        potentialParticles = self.transProbDict.keys()
        for i in range(self.NUM_PARTICLES):
            chosenElem = util.weightedRandomChoice(weights)
            resample[chosenElem] += 1
